...
import './CSS/Settings.css';
import {
  IoSaveOutline, IoRefreshOutline, IoEyeOutline, IoEyeOffOutline, IoInformationCircleOutline,
  IoAdd, IoTrashOutline, IoGlobeOutline, IoLockClosedOutline, IoMailOutline, IoCallOutline, IoPersonOutline
...
const Settings = () => {
  const { tournament, fetchTournamentDetails, backend_URL } = useContext(OrganizerContext);
  const navigate = useNavigate();
  const { id } = useParams();
  ...
  const [settingsData, setSettingsData] = useState(() => ({
    ...
    otp: '456789',
    isPublicTournament: tournament?.type === 'Public',
    seedingOptionInFixtures: tournament?.settings?.seedingOptionInFixtures || false,
  ...
  const removeCustomField = (id) => {
    setSettingsData(prev => {
      const filteredFields = prev.customFields.filter(field => field.id !== id);
      // Prevent deleting all fields, always keep at least one
      return {
        ...prev,
        customFields: filteredFields.length > 0 ? filteredFields : prev.customFields
      };
    });
    setHasUnsavedChanges(true);
  };
          ...
          <div className="settings-section-content">
            ...
            <div className="settings-form-group">
              ...
              <div
                ...
                onClick={() => { window.open(`http://localhost:5173/events/${tournament?._id}`, "_blank"); }}
        ...
        <div className="settings-section">
          ...
          <div className="settings-section-content">
            <div className="settings-custom-fields">
              ...
              <button className="settings-add-field-btn" onClick={addCustomField} type="button">
                <IoAdd className="settings-add-field-icon" />
                Add Custom Field
              </button>
            </div>
